...
const PLAN_FEATURES = {
  ...
  starter: {
    ...
    features: ['Advanced SEO optimization', 'Long-form descriptions']
  },
  pro: {
    maxTags: 15,
    maxDescriptionLength: 500,
    features: ['Premium SEO optimization', 'Long-form descriptions', 'Advanced features']
  }
};
...
async function generateDescription(imageBase64: string, planType: 'free' | 'starter' | 'pro') {
...
Deno.serve(async (req) => {
  ...
  try {
    console.log('Received request to generate description');

    // Parse the request body
    const body = await req.json();
    const { image, userId, usageLimit, isPremium } = body;
    ...
    console.log(`Processing image for ${planType} plan user`);

    const result = await generateDescription(image, planType);
